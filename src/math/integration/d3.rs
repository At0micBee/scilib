...
fn trapz_fn_sub_y(
    ...
    let mut area = 0.0;
    let mut save_previous = trapz_fn_sub_sub_z(function, curr_x, curr_y_lower_bound, z_lower_bound, z_upper_bound, div_z);
    for j in 0..div_z {
        let next_value = trapz_fn_sub_sub_z(function, curr_x, curr_y_lower_bound + (j + 1) as f64 * step, z_lower_bound, z_upper_bound, div_z);
        area += save_previous + next_value;
        save_previous = next_value;
    }
    step * area / 2.0
...
pub fn trapz_fn(
    ...
    let mut area = 0.0;
    let mut save_previous = trapz_fn_sub_y(function, x_lower_bound, y_lower_bound, y_upper_bound, z_lower_bound, z_upper_bound, div_y, div_z);
    for i in 0..div_z {
        let next_value = trapz_fn_sub_y(function, x_lower_bound + (i + 1) as f64 * step, y_lower_bound, y_upper_bound, z_lower_bound, z_upper_bound, div_y, div_z);
        area += save_previous + next_value;
        save_previous = next_value;
    }
    step * area / 2.0
